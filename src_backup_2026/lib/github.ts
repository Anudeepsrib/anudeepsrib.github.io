...
class GitHubService {
  ...
  // Allow octokit to be null
  public readonly octokit: Octokit | null;
  ...
  private constructor() {
    ...
    if (!token) {
      console.warn('GitHub token is not set. Using fallback/mock data for build.');
      this.octokit = null; // Initialize octokit to null if no token
      return; // Constructor implicitly returns 'this', no explicit return value needed
    }
  ...
  async getUserData(username: string): Promise<UserResponse> {
    ...
    if (!this.octokit) {
      console.warn('GitHub token missing, returning mock user data');
      return { data: { login: username, name: username, bio: 'GitHub token missing', public_repos: 0, followers: 0, following: 0 } } as any;
    }
  ...
  async getRepositories(username: string, options: any = {}): Promise<ReposResponse> {
    ...
    if (!this.octokit) {
      console.warn('GitHub token missing, returning empty repos');
      return { data: [] } as any;
    }
  ...
  async getRepositoryCommits(owner: string, repo: string, options: any = {}): Promise<CommitsResponse> {
    ...
    if (!this.octokit) {
      console.warn('GitHub token missing, returning empty commits');
      return { data: [] } as any;
    }
